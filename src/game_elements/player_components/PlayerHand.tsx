// import type { JSX } from "react";
//
// interface playerHandProps {
//   cardsInHand: JSX.Element[];
//   selectCard: (index: number) => void;
// }
//
// export default function PlayerHand(props: playerHandProps) {
//   return (
//     <>
//       <div className="h-[13dvh] flex justify-evenly bg-gradient-to-r from-stone-900/80 to-stone-800/80 border-2 border-stone-700/60 p-6 shadow-2xl">
//         {props.cardsInHand.map((card, index: number) => (
//           <div key={index} onClick={() => props.selectCard(index)}>
//             {card}
//           </div>
//         ))}
//       </div>
//     </>
//   );
// }
//

import type CompleteCard from "../../UI_Elements/CompleteCard";

interface playerHandProps {
  cardsInHand: CompleteCard[];
  selectCard: (index: number) => void;
}

export default function PlayerHand(props: playerHandProps) {
  return (
    <div className="h-[13dvh] flex justify-evenly bg-gradient-to-r from-stone-900/80 to-stone-800/80 border-2 border-stone-700/60 p-6 shadow-2xl">
      {props.cardsInHand.map((card: CompleteCard, index: number) => (
        <div key={index} onClick={() => props.selectCard(index)}>
          {card.renderCard()}
        </div>
      ))}
    </div>
  );
}
